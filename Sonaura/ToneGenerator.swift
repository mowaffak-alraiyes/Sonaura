        ...
        sourceNode = AVAudioSourceNode(format: stereoFormat) { [weak self] _, _, frameCount, audioBufferList -> OSStatus in
            ...
            for frame in 0..<frames {
                ...
                if channels == 1 {
                    ...
                    if frame == 0 && self.playing && !self.hasLoggedPanInfo {
                        print("⚠️ ToneGenerator: Warning - output is MONO, cannot route to specific ear!")
                        self.hasLoggedPanInfo = true
                    }
                } else if channels >= 2 {
                    // Hard panning for hearing tests - completely mute one channel
                    ...
                    // Explicitly write audio to active channel and ZERO to muted channel
                    // This ensures complete isolation (critical for hearing tests)
                    if leftGain > 0.001 {
                        // Left channel active - hard mute right
                        lptr[frame] = Float(sample * leftGain)
                        rptr[frame] = 0.0  // Explicitly zero right channel
                    } else if rightGain > 0.001 {
                        // Right channel active - hard mute left
                        lptr[frame] = 0.0  // Explicitly zero left channel
                        rptr[frame] = Float(sample * rightGain)
                    } else {
                        // Both muted (shouldn't happen, but handle it)
                        lptr[frame] = 0.0
                        rptr[frame] = 0.0
                    }
                    
                    // Debug first frame to verify panning (only once per tone)
                    if frame == 0 && self.playing && !self.hasLoggedPanInfo {
                        let earName = clampedPan <= -0.99 ? "LEFT" : (clampedPan >= 0.99 ? "RIGHT" : "CENTER")
                        print("🎵 ToneGenerator: Pan=\(String(format: "%.2f", clampedPan)) (\(earName)), LeftGain=\(String(format: "%.3f", leftGain)), RightGain=\(String(format: "%.3f", rightGain))")
                        print("   Channel 0 (LEFT) will receive: \(leftGain > 0.001 ? "AUDIO" : "SILENCE (0.0)")")
                        print("   Channel 1 (RIGHT) will receive: \(rightGain > 0.001 ? "AUDIO" : "SILENCE (0.0)")")
                        print("   Source connected directly to main mixer (no intermediate mixer)")
                        // Verify actual buffer values for first frame
                        if leftGain > 0.001 {
                            print("   ✅ LEFT channel active, RIGHT channel explicitly zeroed")
                        } else if rightGain > 0.001 {
                            print("   ✅ RIGHT channel active, LEFT channel explicitly zeroed")
                        }
                        self.hasLoggedPanInfo = true
                    }
                }
